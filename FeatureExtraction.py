...
def feature_extraction(input_folder, output_folder):
    ...
    for root,dirs,files in os.walk(input_folder):
        for filename in files:
            ...
            np.save(os.path.join(dir_MFCC, f"{os.path.splitext(filename)[0]}.npy"), MFCC)
            ...
            np.save(os.path.join(dir_mel, f"{os.path.splitext(filename)[0]}.npy"), mel_spec_db)
            ...
            np.save(os.path.join(dir_chrom, f"{os.path.splitext(filename)[0]}.npy"), chrom)
            ...
            np.save(os.path.join(dir_beat, f"{os.path.splitext(filename)[0]}.npy"), beat_onset_strength_norm)
